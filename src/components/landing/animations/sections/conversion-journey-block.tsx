...
import { Eye, RotateCcw, ShoppingBag } from "lucide-react";
...
const journeySteps: JourneyStep[] = [
    ...
    },
    {
        ...
        title: "Came Back",
        time: "Today",
        status: "Returned via Direct",
        detail: "Saw welcome popup",
    },
    {
        id: "3",
        ...
        title: "Converted",
        time: "$49",
        status: "Used coupon XFAM20",
        detail: "20% off applied",
    },
];

const STEP_DELAYS = [200, 1000, 1800];

export function ConversionJourneyBlock() {
    const blockRef = useRef<HTMLDivElement>(null);
    const blockInView = useInView(blockRef, { amount: 0.8, margin: "20px 0px -10px 0px", once: true });
...
function StepCard({
    ...
    return (
        ...
        >
            ...
            <JourneyCard step={step} isSuccess={isSuccess} />
        </motion.div>
    );
}
...
function JourneyCard({ step, isSuccess = false }: { step: JourneyStep; isSuccess?: boolean }) {
    return (
        <div className={`bg-card w-fit rounded-xl shadow-lg border ${isSuccess ? 'border-[#FE4A23]/50' : 'border-border'}`}>
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                    <div className="flex-1 min-w-0 flex flex-col divide-y divide-border">
                        <div className="flex gap-3 items-center p-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isSuccess ? 'bg-[#FE4A23] text-white' : 'bg-muted text-foreground'}`}>
                                {step.icon}
                            </div>
                            <h4 className="text-sm font-semibold text-foreground">{step.title}</h4>
                            <span className={`text-xs font-medium whitespace-nowrap ${isSuccess ? 'text-[#FE4A23]' : 'text-muted-foreground'}`}>
                                {step.time}
                            </span>
                        </div>
                        <div className="flex items-center justify-between w-full p-3 gap-4">
                            <p className="text-sm text-muted-foreground">{step.status}</p>
                            <span className="text-xs text-muted-foreground whitespace-nowrap">{step.detail}</span>
                        </div>
                    </div>
                </div>
            </div>
