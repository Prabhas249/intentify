...
function DitheredWaves({
  ...
  return (
    <>
      ...
      <SafeEffects colorNum={colorNum} pixelSize={pixelSize} />
...
// Error boundary wrapper for Canvas failures
function CanvasFallback() {
  return (
    <div className="w-full h-full bg-gradient-to-r from-muted/20 via-muted/40 to-muted/20" />
  );
}

// Wrapper to safely handle EffectComposer mounting
function SafeEffects({ colorNum, pixelSize }: { colorNum: number; pixelSize: number }) {
  const [mounted, setMounted] = useState(false);
  const { gl } = useThree();

  useEffect(() => {
    // Only mount effects after gl context is confirmed ready
    if (gl && gl.getContext()) {
      setMounted(true);
    }
  }, [gl]);
